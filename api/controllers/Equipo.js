...
class EquipoController {
  ...
  static getOneEquipo(req, res) {
    Equipo.findOne({ where: { id: req.params.id } })
      .then((equipo) =>
        equipo.activo ? res.send(equipo) : res.send("el equipo no esta activo")
      )
      .catch((err) => res.status(500).send(err));
  }
  ...
  static async addRole(req, res) {
    try {
      const newRole = await Role.findOrCreate({
      ...
      equipo.addRole(newRole).then(() => {
        if (req.body.necesario) {
          RolEnEquipo.Update(
            { necesario: true },
            { where: { equipoId: equipo.id, roleId: newRole.id } }
          )
  ...
  static async getRoles(req, res) {
    try {
      const roles = await RolEnEquipo.findAll({
